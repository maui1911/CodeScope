...
/// Shared, thread-safe handle to the PTY's stdin writer.
...
///
/// Forwards user-facing events through a channel and writes alacritty's
/// PTY responses (DSR, DA, color queries, etc.) directly to the PTY's
/// stdin so the child process doesn't hang waiting for them.
pub struct GpuiEventProxy {
    /// Channel sender for forwarding events to the GPUI application.
    tx: Sender<TerminalEvent>,
    /// PTY stdin writer. Used to reply to `Event::PtyWrite` payloads;
    /// without this, queries like `ESC[6n` from the shell are dropped
    /// and the shell hangs at startup.
    pty_writer: PtyWriter,
}

impl GpuiEventProxy {
    /// Creates a new event proxy with the given channel sender and PTY writer.
    pub fn new(tx: Sender<TerminalEvent>, pty_writer: PtyWriter) -> Self {
        Self { tx, pty_writer }
    }
...
impl EventListener for GpuiEventProxy {
    ...
    fn send_event(&self, event: Event) {
        match event {
            ...
            Event::PtyWrite(data) => {
                // alacritty asks the embedder to write these bytes back
                // to the PTY (DSR cursor position responses, primary/secondary
                // device attributes, etc.). Dropping them deadlocks shells
                // that wait for the response on startup.
...
mod tests {
    ...
    fn test_event_proxy_creation() {
        let (tx, _rx) = channel();
        let _proxy = GpuiEventProxy::new(tx, sink_writer());
    }

    #[test]
    fn test_wakeup_event() {
        let (tx, rx) = channel();
        let proxy = GpuiEventProxy::new(tx, sink_writer());
