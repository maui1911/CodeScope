...
impl TerminalView {
        ...
        R: Read + Send + 'static,
    {
        ...
        // Wrap stdin writer in Arc<Mutex> for thread-safe access. We do this
        // BEFORE the event proxy so the proxy can hold a clone — alacritty's
        // `Event::PtyWrite` (DSR responses, device-attribute replies, etc.)
        // must be written back to the PTY or the child process hangs.
        let stdin_writer = Arc::new(parking_lot::Mutex::new(
            Box::new(stdin_writer) as Box<dyn Write + Send>
        ));

        // Create event proxy for alacritty, with PTY writer for inline replies.
